...
我认为刷题是一个刻意训练的过程，刻意训练包括三项，需求、建模、精化。

其中，需求训练是将问题需求转化为技术需求的训练，要求对给出的问题，进行快速的反应和分析 ；建模训练是构建抽象系统的训练，要求在理解需求的基础上，以构建系统为目标，用规范的语言对系统进行描述；当然，建模并非一次性完成的，需要不断的进行模型迭代，而这个过程就是精化训练的过程。最后，具体的模型系统被构建出来，它采用具体的编程语言进行实现，可以完整的运行，并通过测试用例的模型检验。

总结一下，通过需求训练、模型训练、精化训练，刷题为我们带来了代码能力的提升，让我们能够胜任相关的岗位要求。
