...
The first part of the QoS prediction is the network bias. We assume that the average QoS value $\mu_{R_1,R_2}$ between region $R_1$ and region $R_2$ is 1, and the average QoS value $\mu_{R_1,R_3}$ between region $R_1$ and region $R_3$ is 2. The bias $\bold{b}_1$ between the QoS observed by user $u_1$ and other users in the same region $R_1$ is 0, the bias $\bold{p}_1$ between the Qos provided by service $w_1$ and other services in the same region $R_2$ is 0.1, the bias $\bold{p}'_1$ between the QoS provided by service $w'_1$ and other services in the same region $R_3$ is 0.2. Then the network bias $\mu_{R_1,R_2} + \bold{b}_1 + \bold{p}_1$ of service $w_1$ observed by user $u_1$ is 1.1, and the network bias $\mu_{R_1,R_3} + \bold{b}_1 + \bold{p}'_1$ of service $w'_1$ observed by user $u_1$ is 2.2. 
...
The parameter $\alpha$ is set to 0.5 by default. We add the network bias value and the interaction value between the user and the service, we can get the final QoS prediction value of $w_1$ observed by $u_1$ is 0.9, and the final QoS prediction value of $w'_1$ observed by $u_1$ is 1.5.
...
The latent feature matrices and the bias vectors in Eq2 can be constructed by statistical learning theory. To estimate the values of matrices $U, W$ and vectors $\bold{b},\bold{p}$ , we approximate the original Qos matrix $Q$ with the following objective function, and the minimization formula is as follows:
$$
