...
Since the non-interaction bias from the network environment account for a large proportion of the observed QoS values, it is crucial to model this bias accurately. To achieve this goal, we propose a network biased matrix factorization (NBMF) method for QoS prediction. Our method considers both interaction and non-interaction between users and services, which makes our method adaptable to complex network environments. In this section, we first introduce the proposed NBMF method, then give a simple example to illustrate the improved prediction process, and finally describe the model training and parameter optimization process.
...
If we consider the network bias to be continuous rahter than discrete, then linear regression can be used to predict the network bias between users and services. Following the above idea, we propose a network biased matrix factorization method. Our method packages network bias into a linear regression model while putting the user-service interaction into a matrix factorization model. Thus, the general objective function for NBMF-based QoS prediction method can be derived as:
...
The first term $\alpha \ (\mu_{xy} + \bold{b}_i + \bold{p}_j)$ of the objective function is a linear regression model used to predict the network bias between user $i$ and service $j$. Where, $x$ is the network of user $i$ , $y$ is the network of service $j$ , and $\mu_{xy}$ is the average QoS value between network $x$ and network $y$ . $\bold{b}_i (1\le i\le m)$ denotes the bias between the QoS observed by user $i$ and other users in the same network. $\bold{p}_j (1\le j\le n)$ denotes the bias between the QoS provided by service $j$ and other services in the same network. 
...
The weight $\alpha (0\le\alpha\le 1)$ measures the degree of network bias in our prediction model. $\alpha$ is an adjustable parameter. If $\alpha$ is set to 0, our prediction model does not consider network bias and only uses matrix factorization for prediction。 If $\alpha$ is set to 1, our prediction model does not consider the user-service interaction and only uses linear regression for prediction. In order to investigate the impact of $\alpha$ on our model and find an optimal model, the value of $\alpha$ will be evaluated in Section 5.



## 3.2 Method Example(OK)

改进后的评分预测过程如图3所示，我们以用户$u_1$调用服务$w_1$，$w'_1$为例进行说明。

The QoS prediction of NBMF is shown in Fig. 3, In the following, we will focus on user $u_1$ and services $w_1$, $w'_1$ .
...
The first part of the QoS prediction is the network bias. We assume that the average QoS value $\mu_{R_1,R_2}$ between region $R_1$ and region $R_2$ is 1, and the average QoS value $\mu_{R_1,R_3}$ between region $R_1$ and region $R_3$ is 2. The bias $\bold{b}_1$ between the QoS observed by $u_1$ and other users in $R_1$ is 0, the bias $\bold{p}_1$ between the Qos provided by $w_1$ and other services in $R_2$ is 0.1, the bias $\bold{p}'_1$ between the QoS provided by $w'_1$ and other services in $R_3$ is 0.2. Then the network bias $\mu_{R_1,R_2} + \bold{b}_1 + \bold{p}_1$ between $u_1$ and $w_1$ is 1.1, and the network bias $\mu_{R_1,R_3} + \bold{b}_1 + \bold{p}'_1$ between $u_1$ and $w'_1$ is 2.2.

The next part of the QoS prediction is the matrix factorization, and the detailed procedure can be found in 2.2
...
The next part of the QoS prediction is the matrix factorization, and the detailed procedure can be found in 2.2. The dot product of user u1 and service w1 represents the Qos value of service w1 observed by user u1, which is 0.8. Since the performance of services w1 and w'1 are exactly the same, they have the same degree of latent factors, the QoS value of service w'1 observed by user u1 is also 0.8.



参数 $\alpha$ 默认设置为0.5，我们将用户调用服务的网络偏置值加上只依赖与用户和服务交互作用的预测值，可以得到用户 $u_1$ 调用服务 $w_1$ 的最终Qos预测值为0.9，用户 $u_1$ 调用服务 $w'_1$ 的最终Qos预测值为1.5。

The parameter $\alpha$ is set to 0.5 by default. After weighted sum of the network bias and the interaction between the user and the service, we can get the predicted QoS value of $w_1$ observed by $u_1$ is 0.9, and the predicted QoS value of $w'_1$ observed by $u_1$ is 1.5.



虽然服务$w_1$和服务$w'_1$的性能指标完全相同，但是它们所属区域不同（服务$w_1$位于R2，服务$w'_1$位于R3），用户调用它们的Qos值也会有很大不同。可见我们的方法对复杂的网络环境具备很好的适应性。

Although the performance of services $w_1$ and $w'_1$ are the same, they belong to different regions ($w_1$ is located in $R_2$, $w'_1$ is located in $R_3$), and their QoS values observed by $u_1$ are different. It can be seen that our method is adaptable to complex network environments.



Although the performance of $w_1$ and $w'_1$ are the same, they belong to different regions ($w_1$ is located in $R_2$ , $w'_1$ is located in $R_3$), and their QoS values observed by the user can be very different. It can be seen that our method is adaptable to complex network environments.
...
The latent feature matrices and the bias vectors in Eq. 2 can be constructed by statistical learning theory. To estimate the values of matrices $U, W$ and vectors $\bold{b},\bold{p}$ , we approximate the original Qos matrix $Q$ with the following objective function, and the minimization formula is as follows:
$$
...
$$
其中 $I_{ij}$ 是指示函数，如果用户 $i$ 调用过服务 $j$，则返回1，否则返回0。

$\hat{Q}_{ij}$表示用户 $i$ 调用服务 $j$ 的Qos预测值。

the general objective function for NBMF-based QoS prediction methods can be derived as:

Q is the prediction function as in equation 2

q是来自于公式2的预测函数

为了避免过度拟合得到最优的矩阵 $U、W$ 和向量 $\bold{b}、\bold{p}$ 逼近原始矩阵 $Q$，我们加入了四个与 $U、W$ 和 $\bold{b}、\bold{p}$ 相关的正则项：

Where $I_{ij}$ is the indicator function that returns 1 if user $i$ has invoked service $j$, and 0 otherwise. To avoid overfitting in approximating the original matrix, we add four regular terms related to $U,W$ and $\bold{b}、\bold{p}$. 
$$
\begin{split}
...
\tag{4}
$$
其中$\Vert \cdot \Vert$ 表示 Frobenius 范数[12.26.01]， $\lambda_u$、$\lambda_w$、$\lambda_b$ 和 $\lambda_p$ 四个参数控制正则化程度。目标函数 L 使用二次正则项来最小化平方误差和。因为他不是凸的，所以设计一个算法来找到全局最小是不现实的。我们选择采用随机梯度下降法[41]求解矩阵 $U、W$ 和向量 $\bold{b}、\bold{p}$ 的最优解。For each Qos record observed when user i invokes service j, we have the following updating rules:

Where $\Vert \cdot \Vert$ denotes the Frobenius norm [14], The parameter $\lambda$ controls the degree of regularization. The objective function based on L2-norm as in Eq. (4) is not convex, it is unrealistic to design an algorithm to find the global minimum [15]. Instead, the stochastic gradient descent technique [16] can be employed to find the approximate optimal solution. For each Qos record observed when user i invokes service j, we have the following updating rules:



```
[14] Guaranteed minimum-rank solutions of linear matrix equations via nuclear norm minimization
[15] Factor in the neighbors: Scalable and accurate collaborative filtering
...
where a represents the learning rate of solving the feature matrix and bias vector
...
算法1的主要时间成本在于矩阵 $U、W$ 和向量 $\bold{b}, \bold{p}$ 的更新，在每一轮的迭代中，更新 $U、W$ 需要的时间成本为 $O(sd)$，更新 $\bold{b}、\bold{p}$ 需要的时间成本为 $O(s)$ 。因此NBMF算法的整体时间复杂度可以用 $O(rsd)$ 表示。



The overall optimization procedure of our method is given in Algorithm 1. Let $r$ denote the number of iterations to achieve convergence, let $s$ denote the number of valid invocation records in the original matrix $Q$, and let $d$ denote the dimensions of the user latent feature matrix and the service latent feature matrix. The main time cost of Algorithm 1 lies in the updating of matrices $U,W$ and vectors $\bold{b}, \bold{p}$. In each iteration, updating $U,W$ takes $O(sd)$ time and updating $\bold{b}, \bold{p}$ takes $O(s)$. Thus, the overall time complexity of our method is $O(rsd)$.
